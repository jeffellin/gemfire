...
public class Publish {
	...
	public static void main(String[] args) throws Exception {
			...
			Region<Integer, Employee> region =
					cache.<Integer, Employee>createClientRegionFactory(ClientRegionShortcut.PROXY)
							.create(REGIONNAME);

			// create a set of employee data and put it into the region
			Map<Integer, Employee> employees = createEmployeeData();
		...
		public static Map<Integer, Employee> createEmployeeData() {
			String[] firstNames =
			...
			for (int index = 0; index < firstNames.length; index++) {
				...
				Employee value = new Employee().setFirstName(firstNames[index]).setLastName(lastNames[index]).setEmplNumber(emplNumber).setEmail(email).setSalary(salary).setHoursPerWeek(hoursPerWeek);
		...
		public static void doQueries(ClientCache cache) throws NameResolutionException,
			...
			SelectResults<PdxInstance> results =
					(SelectResults<PdxInstance>) queryService.newQuery(QUERY1).execute();
			printSetOfEmployees(results);

			// Query for all part time employees, and print query results.
			System.out.println("\nExecuting query: " + QUERY2);
			results = (SelectResults<PdxInstance>) queryService.newQuery(QUERY2).execute();
			printSetOfEmployees(results);

			// Query for last name of Jive, and print the full name and employee number.
			System.out.println("\nExecuting query: " + QUERY3);
			results =
					(SelectResults<PdxInstance>) queryService.newQuery(QUERY3).execute(new String[] {"Jive"});
			/*for (Employee eachEmployee : results) {
		...
		private static void printSetOfEmployees(SelectResults<PdxInstance> results) {
			System.out.println("Query returned " + results.size() + " results.");
			for (PdxInstance eachEmployee : results) {
				System.out.println(String.format("Employee: %s", eachEmployee.getObject().toString()));
			}
		}
}
